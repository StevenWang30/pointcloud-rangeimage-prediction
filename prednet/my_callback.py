...
class My_Callback(keras.callbacks.Callback):
    def test_image(self, epoch, validation_data):
        ...
        # *255
        X_test[0:20] = X_test[0:20] * 255
        X_hat[0:20] = X_hat[0:20] * 255
        ...
        for i in range(5):
            ...
            for t in range(1, 7, 1):
                im = np.concatenate((im, X_test[i, t]),
                                          axis=0).astype(np.uint8)
                im = np.concatenate((im, X_hat[i, t]),
                                          axis=0).astype(np.uint8)
    ...
    def my_on_epoch_end(self, epoch, validation_data):
        print("on epoch end: process val data and save: ...")
        # IPython.embed()
        # if(epoch % 10 == 0):
        My_Callback.test_image(self, epoch, validation_data)
